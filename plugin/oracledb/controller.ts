...
import { OracleDBControllerData, OracleDBDataSource } from '@plugin/oracledb/types/data';
import { OracleDBResult, OracleDBExtraResult, OracleDBDetailResult, OracleDBFields, OracleDBRows, OracleDBExecuteResult } from '@plugin/oracledb/types/result';
import { execute, ExecuteResult } from '@plugin/oracledb/utils';
 ...
 * @author William Chan <root@williamchan.me>
 */
export default class OracleDBController extends SingleController<OracleDBControllerData> {
  private command!: string;
  private readonly server!: OracleDBDataSource;
  private readonly result: ResultOptions = {};

  /**
   * @inheritdoc
   */
  public constructor(data: OracleDBControllerData, context: Context, extra?: ControllerExtraConfig) {
    super(data, context, extra);
    this.command = this.data.command;

    // get env data source config
    const server = getDataSourceByServer<OracleDBDataSource>(this.data.serverId, this.context.env.dataSource);
    if (server && server.type === DATA_SOURCE_TYPE.ORACLEDB) {
      this.server = server;
    } else {
      this.setError(new SystemError(`OracleDB Configuration ${this.data.serverId} not found.`));
    }
  }
  ...
  private responseHandler(result: ExecuteResult): void {
    ...
    if (result.rows && result.fields) {
      ...
      const executeResult: OracleDBExecuteResult = { ...result.data };
   ...
   * @returns {ProxyHandler<OracleDBProxyData[]>}
   */
  private createProxyHandler(): ProxyHandler<OracleDBProxyData[]> {
    return {
   ...
   * @inheritdoc
   */
  public async getExtraResult(): Promise<OracleDBExtraResult> {
   ...
   * @inheritdoc
   */
  public async getDetailResult(): Promise<OracleDBDetailResult> {
   ...
   * @param base
   * @param data
   * @returns {OracleDBResult}
   */
  public static createInitResult(base: BaseResult, data: OracleDBControllerData): OracleDBResult {
    const result: OracleDBResult = {
      ...base,
